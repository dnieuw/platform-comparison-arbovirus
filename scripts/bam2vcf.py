...
parser = argparse.ArgumentParser(description='VCF generation script from BAM file')
...
def print_header(ref, outfile):
    ...
    print('##contig=<ID='+ref+'>', file=outfile)
...
##INFO=<ID=MAJOR,Number=1,Type=Integer,Description="Indicates that the variant is the major variant with 1 or not with 0">""", file=outfile)
...
def work(start,stop):
    ...
    pileup = bamfile.pileup(contig=ref, start=start, stop=stop, ignore_orphans=False, min_mapping_quality=0, min_base_quality=0, truncate=True)
    ...
    with open(args.out,"w") as outfile:
        ...
        for result in itertools.chain.from_iterable(resultlist):
            ...
            for n,alt in enumerate(alleles):
                ...
                indel = ';INDEL=0' if (len(alt[0])+len(alt[1]))==2 else ';INDEL=1'
                ...
                print(ref, ref_pos+1, ".", alt[0], alt[1], ".", "PASS", "DP="+num_aln+";AF="+"{:.6f}".format(alt_freq[n])+";DP4="+counts[ref_index]+","+counts[n]+indel+major, sep='\t', file=outfile)